...
with DAG(dag_id='tz_dag', schedule_interval="0 2 * * *", default_args=default_args) as dag:
    ...
    print('[DAG:tz_dag] timezone: {0} - start_date: {1} - schedule_interval: {2} - Lastest execution_date: {3} - next execution_date {4} in UTC - next execution_date {5} in local time'.format(
        dag.timezone,
